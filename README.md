...
Clicking on this link allows you to see the user’s actions leading up to and following the error in a FullStory session replay. In this example, we see our user type the unsearchable term ("Florida") into the search box and submit before they see the Error Boundry screen. The "Application Eeror" event is visible in the event stream on the right-hand side of the screen.
